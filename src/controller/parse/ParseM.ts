...
export class ParseM {
	...
	public static MHelper(math: InsightM, json: any, parser: QueryParser): Promise<MComparison> {
		...
		if (Object.keys(json).length > 1) {
			return Promise.reject(new InsightError("too many keys in M comparison"));
		}
		...
		if (typeof value !== "number") {
			return Promise.reject(new InsightError("number value in M comparison failed to parse"));
		}
		// create MComparison object
		return Promise.resolve(new MComparison(math, field, value));
	}
}
